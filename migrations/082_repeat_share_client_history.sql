-- Migration 082: «Доля повторных продаж» — фикс определения (баг #1556, Серёга)
-- (изначально заведена в репо как 078, переименована Артёмом перед деплоем 56 —
-- номер 078 на сервере занят ad-hoc changelog-файлом деплоя 53, вне этого репо;
-- фактически занято на сервере по 081 включительно)
