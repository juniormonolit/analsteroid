...
export async function GET() {
  ...
  const res = await db.query<SavedReport>(
    `SELECT id, user_login AS "userLogin", report_slug AS "reportSlug", name,
            ...
            COALESCE(deal_filters, '[]'::jsonb) AS "dealFilters",
            ...
            period_dimension AS "periodDimension",
...
function buildCommonFields(body: SavedReportInput, sharedSection: SharedSection): Record<string, unknown> {
  return {
    ...
    metric_borders: JSON.stringify(body.metricBorders ?? {}),
    ...
    deal_filters: JSON.stringify(body.dealFilters ?? []),
    ...
    period_unit: body.periodUnit ?? null,
    period_dimension: body.periodDimension ?? null,
    compare_mode: body.compareMode ?? null,
    ...
    border_mode: body.borderMode ?? null,
  };
}
