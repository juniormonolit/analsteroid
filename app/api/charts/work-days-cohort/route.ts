...
// Когорта «В работе → продажа по дням» (раздел «Графики», задача 2553, 29.07).
// Тот же паттерн валидации/прав, что app/api/charts/called-to-sale-cohort/route.ts
// (тот же движок life table, см. features/charts/engine/lifeTable.ts).
...
export async function POST(req: NextRequest) {
  ...
  if (departmentIds && departmentIds.length === 0) return NextResponse.json({ result: null });
