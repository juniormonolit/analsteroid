...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [runs, events] = await Promise.all([
    db.query<{
      ...
      last_value: string | null; next_check_at: string | null; created_at: string | Date; closed_at: string | Date | null; closed_reason: string | null;
    }>(
      `SELECT r.id::text, r.manager_bitrix_id::text, u.display_name AS name, r.status, r.step, r.start_value, r.last_value,
              to_char(r.next_check_at, 'YYYY-MM-DD') AS next_check_at, r.created_at, r.closed_at, r.closed_reason
    ...
    db.query<{ id: string; manager_bitrix_id: string; name: string | null; kind: string; value: string | null; threshold: string | null; text: string; created_at: string | Date }>(
      `SELECT e.id::text, e.manager_bitrix_id::text, u.display_name AS name, e.kind, e.value, e.threshold, e.text, e.created_at
  ...
  return NextResponse.json({
    runs: runs.rows.map(r => ({
      ...
      startValue: n(r.start_value), lastValue: n(r.last_value), nextCheckAt: r.next_check_at,
    ...
    events: events.rows.map(e => ({
      ...
      value: n(e.value), threshold: n(e.threshold), text: e.text, createdAt: d(e.created_at)!,
    })),
  });
}
