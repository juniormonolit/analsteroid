...
export default async function SummaryLayout({ children }: { children: React.ReactNode }) {
  ...
  // свой срез»). Право section.summary ниже остаётся вторым, более тонким рычагом.
  if (!hasFullManagerAccess(session)) {
    return <AccessDenied reason="Раздел доступен только администраторам" />;
  }
  if (!hasPerm(session, 'section.summary')) {
    return <AccessDenied reason="Раздел «Сводная» — выполнение планов по филиалам и динамика продаж. Доступ выдаёт администратор в «Настройки → Роли»." />;
  }
  return <>{children}</>;
}
