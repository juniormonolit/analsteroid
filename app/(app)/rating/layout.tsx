...
export default async function RatingLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!hasFullManagerAccess(session) && !hasPerm(session, 'section.rating')) {
    return <AccessDenied reason="Раздел «Рейтинг» — общий рейтинг менеджеров. Доступ выдаёт администратор в «Настройки → Матрица прав»." />;
  }
  return <>{children}</>;
}
