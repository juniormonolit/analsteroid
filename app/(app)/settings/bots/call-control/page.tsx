...
const PLACEHOLDERS = '{manager_name} {department} {rop_name} {director_name} {phone} {deal_url} {missed_count} {minutes} {case_id} {recipient_name} {manager_chat_url}';
  ...
  return (
    <div className="p-3 sm:p-6 max-w-5xl flex flex-col gap-8">
      ...
      <section>
        ...
        <p className="text-xs text-[var(--color-text-muted)] mb-2">
          В эскалациях (РОП, директор, собственник, конкретный сотрудник)
          <code className="text-[11px]"> {'{manager_name}'} </code> подставляется как
          <b> кликабельное имя</b> — ведёт прямо в чат с этим менеджером. Самому менеджеру —
          обычный текст, без ссылки. Отдельная строка со ссылкой не нужна: если хочется своя
          формулировка, есть <code className="text-[11px]">{'{manager_chat_url}'}</code> —
          голый адрес чата (в шаблоне для менеджера будет «—»).
        </p>
