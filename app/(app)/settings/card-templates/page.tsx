...
export default function CardTemplatesPage() {
  ...
  const savedSnapshotRef = useRef<{ axes: AxisConfig[]; tiles: TileKey[] } | null>(null);
  ...
  function loadTemplate(key: TemplateKey) {
    ...
    fetch(`/api/settings/card-templates?key=${key}`)
      ...
      .then(d => {
        ...
        const loadedTiles: TileKey[] = d.tiles ?? TILE_CATALOG;
        setAxes(loadedAxes);
  ...
  // ── Каталог для пикера: легаси-оси (карточка, знакомые 8) + полный каталог,
  // сгруппированный по категории; фильтр по поисковой строке (id/имя/категория). ──
  const selectedKeys = useMemo(() => new Set((axes ?? []).map(a => a.metricKey)), [axes]);
  const q = search.trim().toLowerCase();
  const legacyFiltered = useMemo(() => {
    if (!catalog) return [];
    return catalog.legacyAxes.filter(a => !q || a.label.toLowerCase().includes(q));
  }, [catalog, q]);
  const catalogByCategory = useMemo(() => {
    if (!catalog) return [] as [string, CatalogMetric[]][];
    const map = new Map<string, CatalogMetric[]>();
    for (const m of catalog.metrics) {
      if (q && !m.nameRu.toLowerCase().includes(q) && !m.id.toLowerCase().includes(q) && !(m.category ?? '').toLowerCase().includes(q)) continue;
  ...
  const atMax = (axes?.length ?? 0) >= maxAxes;
  ...
  return (
    <div className="p-3 sm:p-6 max-w-5xl">
      ...
      <p className="text-sm text-[var(--color-text-muted)] mb-5">
        ...
        «меньше — лучше» настраиваются на каждой оси, плюс какие плитки итогов
        выводить. Рейтинг считается по весам со страницы «Веса скоринга» — для осей
      ...
      {axes === null || tiles === null || catalog === null ? (
        ...
        <>
          <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr_1fr] gap-6 mb-6">
            {/* Превью-паутина */}
            <div>
            ...
            <div>
              <div className="text-xs font-bold uppercase tracking-wide text-[var(--color-text-muted)] mb-2">
                Каталог метрик{atMax && ' (максимум осей достигнут)'}
              </div>
              <input
                type="text"
                value={search}
              ...
              <div className="border border-[var(--color-border)] rounded-lg max-h-[420px] overflow-y-auto">
                {legacyFiltered.length > 0 && (
                  <div>
                    ...
                    {legacyFiltered.map(a => {
                      const selected = selectedKeys.has(a.metricKey);
                      return (
                        <button
                          key={a.metricKey}
                          ...
                          className={`w-full text-left px-3 py-2 text-sm border-t border-[var(--color-border)] transition-colors flex items-center justify-between gap-2 ${
                            selected ? 'text-[var(--color-text-muted)] bg-[var(--color-bg)]' : atMax ? 'text-[var(--color-text-muted)] opacity-50 cursor-not-allowed' : 'text-[var(--color-text)] hover:bg-[var(--color-bg-hover)]'
                          }`}
                        >
                ...
                {legacyFiltered.length === 0 && catalogByCategory.length === 0 && (
          ...
          <div className="mb-6">
            ...
            <div className="border border-[var(--color-border)] rounded-lg divide-y divide-[var(--color-border)] max-w-md">
