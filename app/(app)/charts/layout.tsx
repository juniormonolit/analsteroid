...
export default async function ChartsLayout({ children }: { children: React.ReactNode }) {
  const session = await getSession();
  if (!session) redirect('/login');
  if (!hasPerm(session, 'section.charts')) {
    return <AccessDenied reason="Раздел «Графики» — конструктор графиков по метрикам и кривые вероятности продажи. Доступ выдаёт администратор в «Настройки → Роли»." />;
  }
  return <>{children}</>;
}
