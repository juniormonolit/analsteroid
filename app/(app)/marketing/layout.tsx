...
export default async function MarketingLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!hasPerm(session, 'section.marketing')) {
    return <AccessDenied reason="Раздел «Маркетинг» — отчёты по источникам обращений и рекламным каналам. Доступ выдаёт администратор в «Настройки → Роли»." />;
  }
  return <>{children}</>;
}
