...
export default async function ChatsLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!hasFullManagerAccess(session) && !hasPerm(session, 'action.deal_chats')) {
    return <AccessDenied reason="Раздел «Чаты» — переписки с менеджерами через бота «Аналитик». Доступ выдаёт администратор в «Настройки → Матрица прав»." />;
  }
  return <>{children}</>;
}
