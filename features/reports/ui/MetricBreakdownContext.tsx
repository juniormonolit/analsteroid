...
export interface BreakdownReportContext {
  ...
  rows: BreakdownRow[];
  /**
   * Все id строк отчёта БЕЗ фильтра поиска — популяция «Итого». Ячейка «Итого»
   * в отчёте по менеджерам считается только по его строкам (движок клиентских
   * метрик получает managerIds строк; collected — суммой по строкам), поэтому
   * разбор для «Итого» обязан резать список тем же набором, а не всем срезом:
   * инцидент 03.09 — «Сумма (Новые клиенты)» расходилась на 11,7 млн из-за
   * 124 первых покупок на аккаунтах не-manager* (РОПы/админы/логист), которые
   * фильтр «тип аккаунта» убирает из строк, но не из базы.
   */
  totalRowIds: string[];
