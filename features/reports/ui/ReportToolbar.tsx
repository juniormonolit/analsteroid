...
export interface ReportToolbarProps {
  ...
  // «Обычная» (п.3а спеки): скрыть попап «Фильтры», «Вид» — остаются «Сохранить»,
  // «Копировать» и «Обновить». ПРАВКА 04.08 (задача 2990): «Сохранить» больше НЕ
  // гейтится Лайт-режимом — сохранение личного варианта отчёта в «Избранное»
  // признано базовой функцией для всех пользователей (РОП/МОП по умолчанию попадают
  // в basic, т.к. effectiveUiMode() в lib/auth/perms.ts даёт «pro» только
  // action.shared_reports.manage-админам — см. WORKLOG). До правки кнопка «Сохранить»
  // у не-новых отчётов в Лайте вообще не рендерилась в DOM.
  basic?: boolean;
  // Точечное исключение из Лайт-гейта на панель настроек (задача 1572, п.5): отчёт,
  // открытый через «Создать отчёт» (isNew), и так уже показывает «Сохранить» (теперь
  // безусловно) — проп оставлен для остальных вызовов ради обратной совместимости,
  // на видимость «Сохранить» больше не влияет.
