...
export interface ChartDrilldownTarget {
  ...
  allCount: number;     // «Все N»
  soldCount: number;    // «Продано M»
  request: ChartDrilldownRequest;
}

async function fetchChartDeals(req: ChartDrilldownRequest, filter: 'all' | 'sold'): Promise<{ deals: Deal[]; total_count: number; total_amount: number }> {
    ...
    <div className="fixed inset-0 z-[60]">
      ...
      <div className={`fixed inset-y-0 right-0 z-[61] w-full sm:w-[70vw] sm:min-w-[720px] sm:max-w-[1200px] bg-[var(--color-bg)] shadow-2xl border-l border-[var(--color-border)] flex flex-col ${closing ? 'slide-panel-out-right' : 'slide-panel-in-right'}`}>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <div className="flex border border-[var(--color-border)] rounded-lg overflow-hidden text-xs">
              <button
                ...
                className={`px-2.5 py-1 transition-colors whitespace-nowrap ${filter === 'all' ? 'bg-[var(--color-accent)] text-[var(--color-text-inverse)]' : 'text-[var(--color-text)] hover:bg-[var(--color-bg-hover)]'}`}
              >
                Все {target.allCount.toLocaleString('ru-RU')}
              </button>
              <button
                onClick={() => setFilter('sold')}
                className={`px-2.5 py-1 transition-colors whitespace-nowrap ${filter === 'sold' ? 'bg-[var(--color-accent)] text-[var(--color-text-inverse)]' : 'text-[var(--color-text)] hover:bg-[var(--color-bg-hover)]'}`}
              >
                Продано {target.soldCount.toLocaleString('ru-RU')}
              </button>
            </div>
        ...
        <div className="flex-1 overflow-hidden">
          <DealsListBody
            fetchOverride={{
              key: [target.request.endpoint, JSON.stringify(target.request.baseBody), filter, target.request.period.from, target.request.period.to,
              ...
              fn: () => fetchChartDeals(target.request, filter),
            }}
            onDealOpen={setOpenDealId}
            emptyLabel={filter === 'sold' ? 'Нет продаж в этой когорте' : 'Нет сделок в этой когорте'}
          />
        </div>
      </div>
