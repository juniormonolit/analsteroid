...
/**
 * «Денег в игре», ₽ = шанс на повтор × ожидаемая сумма следующей отгрузки.
 *
 * ЧТО ЭТО. Обычное матожидание выручки следующей отгрузки в горизонте 180 дней:
 * E[выручка] = P(отгрузка будет) × E[сумма | была]. Величина честная, но это
 * ставка на столе, а НЕ эффект звонка.
 *
 * ЧЕГО ЭТО НЕ ЕСТЬ (важно, иначе цифру читают неправильно):
 *  - не LTV: горизонт один шаг и 180 дней, а не вся жизнь клиента;
 *  - не прибавка от звонка. Выгода звонка = ΔP × сумма, где ΔP — НАСКОЛЬКО
 *    звонок поднимает вероятность. По исследованию 21.09 это 5–13 п.п. в
 *    среднем, а как ΔP различается между заказчиками — мы не знаем. Ранжируя
 *    по P × сумма, мы неявно считаем, что звонок помогает пропорционально
 *    (мультипликативно). Если он помогает на фиксированные п.п., «правильным»
 *    порядком была бы просто ожидаемая СУММА — поэтому она доступна отдельной
 *    сортировкой, а не спрятана.
 */
