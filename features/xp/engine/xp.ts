...
// Титулы РУКОВОДИТЕЛЕЙ — своя шкала (правка владельца 05.08: «РОПы должны быть
// крутыми, а не стажёрами… Алёне точно надо присвоить „Локомотив“, можно всем
// что-то из такой тематики»). Тематика — тяга и мощь: руководитель тянет отдел.
// Нижняя ступень намеренно НЕ уничижительная: даже РОП с нулевым отделом —
// «Бригадир», а не «Стажёр». Уровень руководителя считается по показателям
// ОТДЕЛА (см. задачу «скиллы РОП/директора»), не по личным продажам.
// Шкала задана владельцем дословно (05.08): низ — «Дрезина» (новичок-РОП),
// середина — «Локомотив» («как промежуточное, аналогично ветерану в продажах» —
// поэтому та же ступень 10, что у «Ветерана»), вершина — «Атомный ледокол».
// Промежуточные подобраны в той же теме нарастающей тяги.
export function leaderTitleForLevel(level: number): string {
  if (level >= 30) return 'Атомный ледокол';
  if (level >= 20) return 'Ледокол';
  if (level >= 15) return 'Тягач';
  if (level >= 10) return 'Локомотив';
  if (level >= 5) return 'Тепловоз';
  return 'Дрезина';
}
