  ...
  if (editing) {
    return (
      <div className="flex items-center gap-1 min-w-[120px]">
        ...
        />
        <button onClick={confirm} className="text-[var(--color-positive)] hover:opacity-80"><Check size={12} /></button>
        <button onClick={cancel} className="text-[var(--color-text-muted)] hover:opacity-80"><X size={12} /></button>
      </div>
    );
  }
  ...
  function renderEmployeeRow(emp: Employee) {
    ...
    return (
      <tr key={emp.login} className="hover:bg-[var(--color-bg-hover)] border-b border-[var(--color-border)]">
        <td className="sticky left-0 bg-[var(--color-bg-surface)] z-10 px-3 py-2 w-80 min-w-[320px] border-r border-[var(--color-border)]">
          <div>
      ...
      <table className="border-collapse text-sm" style={{ minWidth: 320 + months.length * colWidth * 2 }}>
        <thead className="sticky top-0 z-20">
          <tr className="bg-[var(--color-bg-surface)] border-b border-[var(--color-border)]">
            <th className="sticky left-0 z-30 bg-[var(--color-bg-surface)] px-3 py-2 w-80 min-w-[320px] text-left text-xs font-semibold text-[var(--color-text-muted)] border-r border-[var(--color-border)] border-b border-[var(--color-border)]" rowSpan={2}>
              Менеджер
            </th>
            {months.map(m => (
              <th
