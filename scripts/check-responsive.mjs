...
const LINE_RULES = [
  ...
  {
    // Задача 2779 (живой инцидент): владелец на iPhone свайпнул полосу вкладок
    // ЛК — уехала ВСЯ СТРАНИЦА вбок, не сама полоса. Причина в CSS-спеке:
    // overflow-y: auto БЕЗ явного overflow-x трактуется браузером как auto по
    // ОБЕИМ осям, если хоть одна не visible — page-root-контейнер с
    // `h-full overflow-y-auto` (устоявшийся паттерн этого проекта — «тот же
    // фикс, что /charts», «тот же паттерн, что SummaryPage») без явного
    // overflow-x-hidden МОЛЧА даёт странице необязательный горизонтальный
    // скролл, если внутри где-то накосячат с шириной (см. min-w-0 ниже —
    // отдельная, НЕ лintable статически часть той же истории, см. комментарий
    // над FILE_RULES). Этот же паттерн — единственная механически проверяемая
    // часть инцидента, дальше не лезем: реальная причина (flex-item без
    // min-w-0, оборачивающий overflow-x-auto ребёнка) требует понимания
    // композиции компонентов, недоступного построчному регэксп-линтеру.
    name: 'page-root-missing-overflow-x-hidden',
    message: 'h-full + overflow-y-auto без overflow-x-hidden — по спецификации CSS страница получает необязательный горизонтальный скролл, если что-то внутри перерастёт ширину (задача 2779)',
    test: (line) => {
      if (!/\bh-full\b/.test(line) || !/overflow-y-auto/.test(line)) return false;
      // overflow-x УЖЕ решён явно (hidden — то, что нам нужно, ИЛИ auto —
      // осознанный выбор горизонтального скролла, например DrilldownDrawer.tsx
      // с таблицей внутри) — не наша забота, ложных срабатываний не хотим.
      if (/overflow-x-(hidden|auto)/.test(line)) return false;
      return true;
    },
  },
];
