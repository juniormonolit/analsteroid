...
const EXACT = {
  ...
  repeat_clients_delivered: 'Клиенты, получившие за период отгрузку, которая для них уже не первая (вторая и далее по всей истории).',
  delivered_deals_count: 'Количество отгруженных сделок (заказов) за период.',
  new_clients_amount: 'Сумма отгрузок за период по новым клиентам (первая отгрузка которых — в этом периоде).',
  repeat_clients_amount: 'Сумма отгрузок за период по повторным клиентам (отгрузка не первая в их истории).',
