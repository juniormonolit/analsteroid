...
# ── Гард схемы (задача 11.08.2026) ────────────────────────────────────────────
# Миграции 144 и 161 неделями не доезжали до прода: магазин отдавал 500 на каждом
# запросе каталога, а экран показывал «Каталог пуст — добавьте первую позицию».
# Поломка, которая выглядит как пустой раздел, глазами не ловится — только
# сверкой схем. Падаем ДО сборки: катить код, который селектит несуществующую
# на проде колонку, нельзя. Осознанный обход — SKIP_SCHEMA_CHECK=1.
if [ "${SKIP_SCHEMA_CHECK:-0}" != "1" ]; then
  echo "==> Сверка схем dev/prod..."
  if ! node scripts/check_schema_drift.mjs; then
    echo "!! Схема прода отстаёт от дева — накатите недостающие миграции ДО деплоя."
    echo "   Осознанный обход: SKIP_SCHEMA_CHECK=1 bash deploy.sh"
    exit 1
  fi
fi
...
# ── Смок схем БД (заведён 11.08.2026) ────────────────────────────────────────
# Живой инцидент: миграции 144 и 161 неделями не доезжали до прода, магазин там
# отдавал 500 на каждом запросе каталога, а экран рисовал «Каталог пуст» — то
# есть поломка выглядела как пустая витрина и никем не замечалась. Глазами такое
# не ловится, только сравнением схем. Падаем ДО сборки: катить код, который
# селектит несуществующие колонки, нельзя.
# Осознанный обход — ALLOW_SCHEMA_DRIFT=1 (например, миграция намеренно только на деве).
if [ "${ALLOW_SCHEMA_DRIFT:-0}" != "1" ]; then
  echo "==> Сверка схем dev ↔ prod..."
  if ! node scripts/check_schema_drift.mjs; then
    echo "!! Схема прода отстаёт от дева — сначала накатите миграции на прод."
    echo "   Осознанный обход: ALLOW_SCHEMA_DRIFT=1 bash deploy.sh"
    exit 1
  fi
fi
