...
const PUBLIC = [
  ...
  '/tv',
  // Дашборд «Сегодня по компании» (решение владельца 14.09: «/today открывался без
  // пароля») — та же модель, что у ТВ: страница без сессии, наружу закрыта только от
  // роботов и лимитом на IP (app/api/tv/dashboard).
  '/today',
  // robots.txt обязан отдаваться без сессии (иначе 307 на /login и роботы его не видят).
  '/robots.txt',
  // Воркер MapLibre (public/maplibre, кладётся на сборке): его грузит не страница,
  // а конструктор Worker — редирект на /login он «съедает» молча, и карта остаётся
  // пустой без единой ошибки. Ровно это и случилось 21.09: сам фикс воркера был
  // верным, но файлы отдавались 307, пока их не внесли сюда. Статика без тайн:
  // это код библиотеки, а не данные.
  '/maplibre',
];
