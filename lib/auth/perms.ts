...
export const PERM_SECTIONS = [
  ...
  // «Карта объектов» (задача владельца 21.09) — адреса доставки сделок на
  // карте с фильтрами и дрилл-дауном. Паттерн закрытого раздела
  // (section.offload): по умолчанию только супер-админ, роли — явно.
  { key: 'section.map', label: 'Карта объектов' },
  ...
  // «Телевизоры» (ТЗ владельца 07.09) — ТВ-дашборды отделов: экраны, привязка
  // телевизоров, бегущие строки и рассылки. Закрытый раздел по паттерну
  ...
  // «РОП — сегодня» (задача #6446, 14.09) — авторизованная персональная копия
  // /today: РОП/Директор видят свой отдел/филиал, рядовой менеджер — только
  // себя (features/tv/engine/access.ts:ropScope). Закрытый раздел по паттерну
  // section.tv: по умолчанию только супер-админ, остальным роль выдаёт
  // владелец явно через «Настройки → Роли» — какие роли получают доступ по
  // умолчанию ещё не решено (см. открытые вопросы в отчёте задачи).
  { key: 'section.rop_today', label: 'РОП — сегодня' },
...
export const PERM_ACTIONS = [
  ...
  // Задача #6260 (Серёга, санкция 11.09): «снять сделку с продажи» — ручная очистка
  // sold_at у сделки, ошибочно поставленной в продажу. По умолчанию НЕ у РОПа —
  // тот же паттерн, что action.subscriptions.view_all: выдаётся явно ролям уровня
  // «Директор»+ через «Настройки → Матрица прав» (таблица «Пункты меню „Ещё“»).
  // sold_at НЕ очищается автоматически ни при каких условиях (решение владельца
  // 11.09: залипание — защита рейтинга от накрутки) — это единственный путь.
  { key: 'action.deals.unsell', label: 'Снятие сделки с продажи (ручная очистка sold_at)' },
  // Задача #6479 (баг Серёги: «/rop все аккаунты видят все, а не только своё»).
  // Причина: полный (не урезанный по отделам) вид /rop решался
  // hasFullManagerAccess() — тем же правом, что «Администратор» получает через
  // джокер section.* (полный доступ КО ВСЕМ РАЗДЕЛАМ приложения). На проде роль
  // «Администратор» носят не только дирекция, но и маркетолог, разработчик
  // («Отдел развития») и тестовый аккаунт в «Отделе продаж» — все они видели
  // ВСЮ компанию на /rop только потому, что им когда-то дали доступ к настройкам/
  // разделам приложения. Это разные вещи: «есть доступ к разделу» ≠ «руководство
  // компании, видит данные всех отделов». Теперь это отдельное, ПОФАМИЛЬНОЕ право
  // (как action.deals.unsell) — джокер section.* его НЕ покрывает, выдаётся явно
  // через «Настройки → Роли»/«Матрица прав» тем, кто действительно должен видеть
  // компанию целиком. По умолчанию ни у одной роли нет (безопасный дефолт: нет
  // явного «руководство» → урезанный/пустой вид, не «всё»), см.
  // features/tv/engine/access.ts:ropFullAccess().
  { key: 'action.rop_today.full_access', label: '«РОП — сегодня»: видеть всю компанию (не только свою зону)' },
] as const;
...
export const MORE_MENU_PERMS: ReadonlyArray<{ key: PermKey; label: string }> = [
  ...
  { key: 'section.widget_constructor', label: 'Виджеты' },
  // «Снять с продажи» (задача #6260) — action.*, как «Чаты»: гейт по действию,
  // не по section.*, джокер «Все разделы» его НЕ покрывает.
  { key: 'action.deals.unsell', label: 'Снять с продажи' },
];
