// Сборка спецификации «Моего отчёта» — ЕДИНАЯ точка для двух потребителей:
//   • POST /api/my-report — конструктор в интерфейсе (клиент сам анимирует цифры
//     из ReportSpec, поэтому отдаём структуру, а не текст);
//   • lib/jobs/reportSchedules.ts — авторассылка сохранённых шаблонов ботом
//     «Аналитик» по расписанию (задача владельца 09.09): тот же spec →
//     buildReportText → сообщение в личку.
// До 09.09 вся логика жила прямо в роуте; вынесена сюда без изменения поведения,
// чтобы рассылка не стала второй, расходящейся реализацией отчёта.
//
...
/** Ошибка входных данных отчёта — роут переводит её в HTTP-статус, раннер
 *  расписаний пишет текст в last_error расписания. */
export class MyReportInputError extends Error {
  constructor(message: string, public readonly status: number) { super(message); }
}

export { EntityAccessError };
...
/** Собрать ReportSpec «Моего отчёта» глазами `session` (доступ к отделам/филиалам
 *  проверяется как у самого пользователя). `body` — состояние конструктора / шаблона:
 *  { period, entities, metricIds, date?, title?, entityAliases?, metricAliases?, showTotal? }. */
export async function buildMyReportSpec(
  session: SessionUser,
  body: Record<string, unknown>,
): Promise<{ spec: ReportSpec; meta: { date: string; period: PeriodKey } }> {
  ...
  if (!entityInput) {
    throw new MyReportInputError(`entities: 1..${MAX_ENTITIES} сущностей вида {kind:"self"|"department"|"branch", id}`, 400);
  }
  ...
  if (metricIdsRaw.length === 0 || metricIdsRaw.length > MAX_METRICS) {
    throw new MyReportInputError(`metricIds: 1..${MAX_METRICS} идентификаторов метрик`, 400);
  }
  ...
  try {
    ...
    if (err instanceof EntityAccessError) throw new MyReportInputError(err.message, 403);
    throw err;
  }
  ...
  if (selected.length === 0) {
    throw new MyReportInputError('Ни одна из запрошенных метрик не найдена в каталоге', 400);
  }
  ...
  return { spec, meta: { date: dateStr, period } };
}
